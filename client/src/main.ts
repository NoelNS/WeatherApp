...
const fetchWeather = async (cityName: string) => {
  ...
  renderCurrentWeather(weatherData);//"look into fixing this error with passing weather or an array"
  renderForecast(weatherData.list);
};
...
const renderCurrentWeather = (currentWeather: any): void => {
  ...
  const currentDate = currentWeather.list[0];
  const city = currentWeather.city.name;
  const date = new Date(currentDate.dt*1000).toLocaleDateString();
  const icon = currentDate.weather[0].icon;
  const iconDescription = currentDate.weather[0].description;
  const tempF = currentDate.main.temp_max;
  const windSpeed = currentDate.wind.speed;
  const humidity = currentDate.main.humidity;
...
const renderForecastCard = (forecast: any) => {
  ...
  const tempF = forecast.main.temp_max;
